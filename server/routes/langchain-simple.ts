...
import { simpleAssistant } from "../langchain-service-simple";
...
router.post("/chat", async (req, res) => {
  try {
    ...
    // Process the message with the simple assistant
    const response = await simpleAssistant.processMessage(message, history);
...
router.post("/voice", upload.single('audio'), async (req, res) => {
  try {
    ...
    const response = await simpleAssistant.processVoice(transcription, history);
...
router.post("/analyze-image", upload.single('image'), async (req, res) => {
  try {
    ...
    const response = await simpleAssistant.analyzeImage(imageDescription, prompt, history);
...
router.post("/analyze-document", upload.single('document'), async (req, res) => {
  try {
    ...
    // Process the message
    const response = await simpleAssistant.processMessage(
